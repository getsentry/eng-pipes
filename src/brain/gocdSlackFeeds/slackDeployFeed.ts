...
export class SlackDeployFeed {
  ...
  getShaBlock(pipeline): KnownBlock | undefined {
    ...
    return {
      ...
      elements: [
        ...
        {
          type: 'mrkdwn',
          text: `<https://github.com/${match.org}/${match.repo}/commits/${modification.revision}|${match.repo} @ ${sha}>`,
        },
      ],
    };
  }

  stageMessage(pipeline: GoCDPipeline) {
  ...
  stageEmoji(pipeline: GoCDPipeline) {
    const stage = pipeline.stage;
    switch (stage.result.toLowerCase()) {
      case 'passed':
        return '✅';
  ...
  getMessageBlocks(pipeline): Array<KnownBlock> {
    const blocks: Array<KnownBlock> = [
      {
        type: 'section',
        text: {
          type: 'mrkdwn',
          text: `*${pipeline.group}/${pipeline.name}*`,
        },
      },
    ];
  ...
  stageBlock(pipeline): KnownBlock {
    ...
    return {
      ...
      elements: [
        {
          type: 'mrkdwn',
          text: `${this.stageEmoji(pipeline)} *${pipeline.stage.name}*`,
        },
        {
          type: 'mrkdwn',
          text: `<${stageOverviewURL}|${this.stageMessage(pipeline)}>`,
        },
      ],
    };
  }
  ...
  async newSlackMessage(refId, pipeline: GoCDPipeline) {
